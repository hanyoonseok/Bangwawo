...
public class EnrolController {
    ...
    @GetMapping("/class/{id}")
    public ResponseEntity<List<Enrol>> findByClassRoom(@PathVariable("id") Long cId) throws Exception{
        return new ResponseEntity<>(enrolService.findByClassRoom(cId) ,HttpStatus.OK);
    }
    @ApiOperation("학생이 수강신청한 수업 받아오기")
    @GetMapping("/student/{id}")
    public ResponseEntity<List<Enrol>> findByStudent(@PathVariable("id") Long sId) throws Exception{
